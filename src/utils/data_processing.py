...
class DataProcessor:
    ...
    def visualize_token_count(self, dictionaries):
        ...
        plt.subplot(311)
        plt.hist(x, range=(0, max(x)), color='red', bins=30)
        plt.xlabel('Text length')
        plt.yscale('log')
        plt.subplot(312)
        plt.hist(y, range=(0, max(y)), color='blue', bins=30)
        plt.xlabel('Ingress length')
        plt.ylabel('Occurences')
        plt.yscale('log')
        plt.subplot(313)
        plt.hist(z, range=(0, max(z)), color='green', bins=30)
        plt.xlabel('Ingress/text length ratio')
        plt.yscale('log')
        plt.show()
